...
migrate_up:
	migrate -path=./migrations -database "${DB_ENGINE}://${DATABASE_DSN}" -verbose up

migrate_down:
	@read -p "Number of migrations you want to rollback (default: 1): " NUM; NUM=$${NUM:-1}; \
		migrate -path=./migrations -database "${DB_ENGINE}://${DATABASE_DSN}" -verbose down $${NUM}
...
force_migrate_up:
	@read -p "Enter the version to force migrate up: " VERSION; \
		...
		if [ $${CONFIRM} = "yes" ]; then \
			migrate -path=./migrations -database "${DB_ENGINE}://${DATABASE_DSN}" -verbose force $${VERSION}; \
		else \
